...
const FloatingParticles = ({
  ...
  useEffect(() => {
    ...
    const applyTouchInteraction = (particle: Particle) => {
      if (!(enableMouse || enableTouch) || !touchRef.current.isActive) return;
    ...
    const applyMultiTouchInteraction = (particle: Particle) => {
      if (!enableTouch || touchRef.current.touches.size <= 1) return;
    ...
    const updateParticlePosition = (particle: Particle, finalSpeed: number, deltaTime: number) => {
    ...
    const handleBoundaries = (particle: Particle, canvasWidth: number, canvasHeight: number) => {
    ...
    const updateParticleEffects = (particle: Particle, canvasWidth: number) => {
      // Pulse animation
      if (enablePulse && (!deviceRef.current.isMobile || performanceRef.current.adaptiveQuality > 0.5)) {
        particle.pulse += particle.pulseSpeed;
        particle.opacity = particle.baseOpacity + Math.sin(particle.pulse) * 0.2;
      }
      ...
      if (enableTrails && performanceRef.current.adaptiveQuality > 0.4) {
        ...
        particle.trail.forEach((point) => { point.opacity *= 0.9; });
      }
    ...
    const updateParticle = (
      ...
      applyMultiTouchInteraction(particle);

      const finalSpeed = deviceRef.current.reducedMotion ? adaptiveSpeed * 0.3 : adaptiveSpeed;
      
      updateParticlePosition(particle, finalSpeed, deltaTime);
      handleBoundaries(particle, canvasWidth, canvasHeight);
      updateParticleEffects(particle, canvasWidth);
    };
    ...
    // Helper functions for connection drawing
    const getConnectionConfig = () => {
      ...
      return {
        quality,
        isMobile,
        qualityThreshold: isMobile ? 0.4 : 0.6,
        maxConnections: Math.floor(particlesRef.current.length * (isMobile ? 0.2 : 0.3) * quality),
        connectionStep: isMobile ? 2 : 1,
        adjustedConnectionDistance: isMobile ? connectionDistance * 0.8 : connectionDistance
      };
    };

    const drawConnection = (ctx: CanvasRenderingContext2D, particle: Particle, other: Particle, config: ReturnType<typeof getConnectionConfig>) => {
      const dx = particle.x - other.x;
      const dy = particle.y - other.y;
      const distance = Math.sqrt(dx * dx + dy * dy);

      if (distance < config.adjustedConnectionDistance) {
        const connectionOpacity = ((config.adjustedConnectionDistance - distance) / config.adjustedConnectionDistance) * (config.isMobile ? 0.1 : 0.15);

        ctx.save();
        ctx.globalAlpha = connectionOpacity * opacity;
        ctx.strokeStyle = particle.color;
        ctx.lineWidth = config.isMobile ? 0.3 : 0.5;
        ctx.beginPath();
    ...
    const drawConnections = (ctx: CanvasRenderingContext2D) => {
      ...
      if (config.quality < config.qualityThreshold) return;

      let connectionCount = 0;

      for (let i = 0; i < particlesRef.current.length && connectionCount < config.maxConnections; i += config.connectionStep) {
        const particle = particlesRef.current[i];

        for (let j = i + config.connectionStep; j < particlesRef.current.length && connectionCount < config.maxConnections; j += config.connectionStep) {
          const other = particlesRef.current[j];
          if (drawConnection(ctx, particle, other, config)) {
            connectionCount++;
          }
        }
      }
