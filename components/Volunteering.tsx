...
const getShapeSize = (index: number): string => {
  if (index % 3 === 0) return "w-8 h-8";
  if (index % 3 === 1) return "w-6 h-6";
  return "w-4 h-4";
};

const getShapeColor = (index: number): string => {
  if (index % 4 === 0) return "bg-purple-500";
  if (index % 4 === 1) return "bg-pink-500";
  if (index % 4 === 2) return "bg-cyan-500";
  return "bg-orange-500";
};

// GeometricShapes component
const GeometricShapes = ({ shapeCount = 20 }) => {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {Array.from({ length: shapeCount }).map((_, index) => (
        <div
          key={`shape-${index}-${Math.random().toString(36).slice(2)}`}
          className={`absolute rounded-full ${getShapeSize(index)} ${getShapeColor(index)} opacity-20 animate-float`}
          style={{
  ...
  return (
    ...
    >
      {Array.from({ length: particleCount }).map((_, index) => (
        <div
          key={`particle-${index}-${Math.random().toString(36).slice(2)}`}
...
export default function Volunteering() {
  ...
  const totalBeneficiaries = volunteeringExperience.reduce((sum, exp) => {
    const match = /\d+/.exec(exp.beneficiaries);
    return sum + (match ? parseInt(match[0]) : 0);
  }, 0);

  const totalMonths = volunteeringExperience.reduce((sum, exp) => {
    const match = /\d+/.exec(exp.duration);
    return sum + (match ? parseInt(match[0]) : 0);
  }, 0);
        ...
        <div className="mb-12 sm:mb-16 md:mb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
            {volunteeringExperience.map((experience, index) => (
              <article
                key={experience.organization.replace(/\s+/g, '-').toLowerCase()}
                  ...
                  <ul className="space-y-1">
                    {experience.contributions
                      ...
                      .map((contribution, cIndex) => (
                        <li
                          key={`${contribution.slice(0, 20).replace(/\s+/g, '-')}-${cIndex}`}
