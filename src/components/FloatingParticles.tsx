...
const FloatingParticles = ({
  ...
  const updatePerformance = useCallback(() => {
    ...
    if (deltaTime > 0) {
      ...
      const isSlowConnection = ['slow-2g', '2g'].includes(deviceRef.current.connectionType);
      
      // More aggressive quality reduction for mobile/low battery
      if (performanceRef.current.fps < targetFPS * 0.7 || isLowBattery || isSlowConnection) {
        performanceRef.current.adaptiveQuality = Math.max(0.2, performanceRef.current.adaptiveQuality * 0.9);
        performanceRef.current.skipFrames = Math.min(3, performanceRef.current.skipFrames + 1);
      } else if (performanceRef.current.fps > targetFPS * 1.1 && !isLowBattery) {
        performanceRef.current.adaptiveQuality = Math.min(1, performanceRef.current.adaptiveQuality * 1.005);
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    if (!canvasRef.current || !enableMouse || deviceRef.current.isTouch) return;
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      const dpr = mobileOptimizations ? Math.min(deviceRef.current.pixelRatio, 1.5) : deviceRef.current.pixelRatio;
    ...
    const updateParticle = (particle: Particle, deltaTime: number, canvasWidth: number, canvasHeight: number) => {
    ...
    const drawParticle = (ctx: CanvasRenderingContext2D, particle: Particle) => {
      ...
      if (enableTrails && particle.trail.length > 1 && quality > (isMobile ? 0.3 : 0.5)) {
    ...
    const drawConnections = (ctx: CanvasRenderingContext2D) => {
      ...
      const maxConnections = Math.floor(particlesRef.current.length * (isMobile ? 0.2 : 0.3) * quality);
      let connectionCount = 0;
      const connectionStep = isMobile ? 2 : 1; // Skip some particles on mobile

      for (let i = 0; i < particlesRef.current.length && connectionCount < maxConnections; i += connectionStep) {
        const particle = particlesRef.current[i];
        
        for (let j = i + connectionStep; j < particlesRef.current.length && connectionCount < maxConnections; j += connectionStep) {
          const other = particlesRef.current[j];
          const dx = particle.x - other.x;
          const dy = particle.y - other.y;
          const distance = Math.sqrt(dx * dx + dy * dy);
          
          const adjustedConnectionDistance = isMobile ? connectionDistance * 0.8 : connectionDistance;
          
          if (distance < adjustedConnectionDistance) {
            const connectionOpacity = ((adjustedConnectionDistance - distance) / adjustedConnectionDistance) * (isMobile ? 0.1 : 0.15);
    ...
    const animate = (currentTime: number) => {
      ...
      if (performanceRef.current.skipFrames > 0 && 
          performanceRef.current.frameCount % (performanceRef.current.skipFrames + 1) !== 0) {
      ...
      const deltaTime = currentTime - (performanceRef.current.lastFrameTime || currentTime);
  ...
  }, [particleCount, colors, speed, connectionDistance, enableTrails, enablePulse, enableGlow, enableMouse, enableTouch, particleSize, opacity, mobileOptimizations, handleMouseMove, handleMouseLeave, handleTouchStart, handleTouchMove, handleTouchEnd, updatePerformance, detectDevice, getOptimizedParticleCount]);
