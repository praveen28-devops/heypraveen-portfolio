  ...
  const volunteeringExperience = [
    {
      ...
      description: 'Supporting community events and elder care initiatives through hands-on volunteering and community engagement.',
      icon: BookOpen,
      contributions: [
      ...
      impact: 'Improved quality of life for 50+ elderly residents and promoted community bonding across age groups.',
      skills: ['Community Engagement', 'Event Planning', 'Elder Care', 'Social Work'],
    ...
    },
    {
      ...
      description: 'Educating and mentoring tribal students in underserved communities, focusing on academic excellence and personal development.',
      icon: Heart,
      contributions: [
      ...
      impact: 'Empowered 30+ rural tribal students with knowledge and confidence to pursue higher education and career opportunities.',
      skills: ['Education', 'Mentorship', 'Rural Development', 'Youth Empowerment'],
  ...
  return (
    ...
    >
      ...
      <div className="sr-only">
        ...
        <p>Community volunteer with {totalMonths}+ months of experience helping {totalBeneficiaries}+ people through education and elder care programs in Tamil Nadu, India.</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Section Header */}
        <header className={`text-center mb-8 sm:mb-12 md:mb-16 transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'opacity-0 translate-y-10'}`}>
          <h2 
          ...
          <div className="w-12 sm:w-16 md:w-20 lg:w-24 h-0.5 sm:h-1 bg-gradient-to-r from-pink-400 to-purple-400 mx-auto rounded-full" aria-hidden="true"></div>
          <p 
            className="text-sm sm:text-base md:text-lg text-slate-400 mt-4 sm:mt-6 max-w-3xl mx-auto"
            itemProp="description"
          >
            Making a positive impact through community engagement, education, and knowledge sharing across Tamil Nadu. 
            <span className="font-semibold text-purple-400"> {totalBeneficiaries}+ lives impacted</span> through dedicated volunteer work.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-4 sm:gap-6 mt-6 sm:mt-8">
            <div className="bg-slate-800/30 backdrop-blur-sm border border-purple-500/20 px-3 sm:px-4 py-2 rounded-lg">
              <span className="text-purple-400 font-bold text-lg sm:text-xl">{totalBeneficiaries}+</span>
            ...
            <div className="bg-slate-800/30 backdrop-blur-sm border border-pink-500/20 px-3 sm:px-4 py-2 rounded-lg">
              <span className="text-pink-400 font-bold text-lg sm:text-xl">{totalMonths}+</span>
              <p className="text-xs sm:text-sm text-slate-400">Months of Service</p>
            </div>
            <div className="bg-slate-800/30 backdrop-blur-sm border border-cyan-500/20 px-3 sm:px-4 py-2 rounded-lg">
              <span className="text-cyan-400 font-bold text-lg sm:text-xl">2</span>
            ...
            {volunteeringExperience.map((experience, index) => (
              ...
              >
                <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-br ${experience.color} rounded-lg sm:rounded-xl flex items-center justify-center mb-2.5 sm:mb-3 md:mb-4 group-hover:scale-110 group-hover:shadow-lg transition-all duration-300`}>
                  <experience.icon className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 text-white group-hover:animate-pulse" aria-hidden="true" />
                </div>
                ...
                <div className="mb-2.5 sm:mb-3 md:mb-4">
                  <h5 className="text-xs font-semibold text-white mb-1.5 sm:mb-2">Key Contributions</h5>
                  <ul className="space-y-1" role="list">
                    {experience.contributions.slice(0, 3).map((contribution, cIndex) => (
                      <li key={cIndex} className="flex items-start space-x-1.5">
                        <div className="w-1 h-1 bg-purple-400 rounded-full mt-1 flex-shrink-0" aria-hidden="true"></div>
                        <p className="text-xs text-slate-400 line-clamp-2">{contribution}</p>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="flex justify-between text-xs text-slate-500">
                  <span>Duration: <strong className="text-purple-400">{experience.duration}</strong></span>
                  <span>Beneficiaries: <strong className="text-pink-400">{experience.beneficiaries}</strong></span>
                </div>
                
                {/* Hidden Structured Data */}
                <div className="sr-only">
                  <span itemProp="startDate">{experience.period.split(' - ')[0]}</span>
                  <span itemProp="endDate">{experience.period.includes('Present') ? new Date().getFullYear() : experience.period.split(' - ')[1]}</span>
                </div>
              </article>
            ))}
          </div>
