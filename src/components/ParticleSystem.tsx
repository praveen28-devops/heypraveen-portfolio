...
const ParticleSystem: React.FC<ParticleSystemProps> = ({ 
  ...
  const handleInteraction = useCallback((clientX: number, clientY: number) => {
    ...
    const rect = document.querySelector('.particle-container')?.getBoundingClientRect();
    if (!rect) return;
    ...
    setTimeout(() => {
      ...
      setParticles(prev => prev.map(p => ({ ...p, touchEffect: false })));
    }, isMobile ? 1000 : 800);
    ...
    const interval = setInterval(() => {
      setParticles(prev => prev.map(particle => {
        ...
        return {
          ...
          vx: newVx,
          vy: newVy,
          opacity: particle.touchEffect ? particle.opacity : Math.max(particle.opacity - 0.002, 0.3)
        };
      }));
    }, isMobile ? 100 : 50); // Much slower on mobile for performance
      ...
      {particles.map((particle) => (
        <div
          ...
          style={{
            ...
            transform: 'translate(-50%, -50%)',
            transition: particle.touchEffect ? 'all 0.3s ease' : 'opacity 0.5s ease',
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                onChange={(e) => setCurrentIntensity(e.target.value as 'low' | 'medium' | 'high')}
            ...
            <div className="flex items-center gap-2">
              <label className="text-white text-sm font-medium">Interactive:</label>
              <button
          ...
          <p className="text-white/70 text-xs mt-3 text-center">
            {isInteractive ? 'Touch or hover to interact with particles' : 'Particles are in display-only mode'}
          </p>
        </div>
        ...
        <div className="relative h-96 md:h-[500px] bg-black/20 rounded-lg overflow-hidden">
          <ParticleSystem
            ...
            colorPalette={['#3b82f6', '#8b5cf6', '#06b6d4', '#10b981', '#f59e0b', '#ef4444']}
          />
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
            <h3 className="text-white font-semibold mb-2">Mobile Optimizations</h3>
